...
RUN apt update && \
    apt install -y --no-install-recommends --no-install-suggests \
        ...
        python-pip python-setuptools build-essential python-all-dev libffi-dev libcurl4-openssl-dev && \
    pip install --no-cache-dir --upgrade plexapi pyopenssl && \
# clean up
    apt purge -y python-pip python-setuptools build-essential python-all-dev libffi-dev libcurl4-openssl-dev && \
    apt autoremove -y && \
    apt clean && \
